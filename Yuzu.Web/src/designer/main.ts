    ...
    try {
        ...
        if (isLoadingExisting && isLoadingExisting.value === "true" && initialCanvasDataContainer) {
            ...
            try {
                ...
                // Get the JSON and background title from the data attributes
                ...
                if (jsonData) {
                    ...
                    // If we have a background title, set it in the UI and apply the background image
                    if (backgroundTitle) {
                        console.log('[Debug] Loading design with background title:', backgroundTitle);
                            ...
                            try {
                                ...
                                if (response.ok) {
                                    ...
                                    if (result.success && result.backgrounds && Array.isArray(result.backgrounds)) {
                                        // Log all backgrounds to help with debugging
                                        console.log(`[Debug] Received ${result.backgrounds.length} backgrounds from API`);
                                        result.backgrounds.forEach((bg: any, index: number) => {
                                            console.log(`[Debug] Background ${index}: Name=${bg.name}, Title=${bg.title || 'NULL'}`);
                                        });
                                        ...
                                        const matchingBackground = result.backgrounds.find(
                                            (bg: any) => {
                                                ...
                                                const matches = bgTitle === normalizedSearchTitle;
                                                console.log(`[Debug] Comparing: "${bgTitle}" === "${normalizedSearchTitle}" = ${matches}`);
                                                return matches;
                                            }
                                        );
                                        
                                        if (matchingBackground) {
                                            console.log(`[Debug] Found matching background with title "${backgroundTitle}"`);
                                            console.log(`[Debug] Match details: Name=${matchingBackground.name}, Title=${matchingBackground.title}, URL=${matchingBackground.fullImageUrl}`);
                                            
                                            // Make sure we have a valid URL before applying
                                            if (matchingBackground.fullImageUrl) {
                                                // Use the applyBackground function to apply the image
                                                applyBackground(matchingBackground.fullImageUrl, matchingBackground.title || matchingBackground.name);
                                                
                                                // Also set as the selected background title
                                                selectedBackgroundTitle = matchingBackground.title || matchingBackground.name;
                                            } else {
                                                console.error(`[Debug] Matching background has no fullImageUrl`);
                                            }
                                        } else {
                                            console.error(`[Debug] Could not find background with title "${backgroundTitle}"`);
                                            
                                            // Fallback: if we can't find by title, try using the first background
                                            if (result.backgrounds.length > 0) {
                                                console.log(`[Debug] Using first available background as fallback`);
                                                const fallbackBg = result.backgrounds[0];
                                                applyBackground(fallbackBg.fullImageUrl, fallbackBg.title || fallbackBg.name);
                                                selectedBackgroundTitle = fallbackBg.title || fallbackBg.name;
                                            }
                                        }
                                    } else {
                        ...
                        console.log('[Debug] No background title found for this design');
                    }
                } else {
