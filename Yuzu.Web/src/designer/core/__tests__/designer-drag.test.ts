...
import { DragType, ResizeHandlePosition, Point, WidgetType, DragState } from '../../types.js';
import { MoveWidgetsCommand, ResizeWidgetCommand } from '../../commands/basic-commands.js';
import { Widget } from '../../widget.js';

// Create a mock for PropertiesManager
...
class TestDesigner extends Designer {
  constructor(canvasElementId: string) {
    ...
    (this as any).selectionManager = {
      ...
      isWidgetSelected: jest.fn().mockImplementation((id) => {
        ...
        if ((this as any).expectedSelectedId === id) {
          return true;
        }
        return false;
      }),
  ...
  public getWidgetPosition(id: string): Point | undefined {
  ...
  public getWidgetSize(id: string): { width: number, height: number } | undefined {
    const widget = this.widgets.get(id);
    return widget ? {
  ...
  public isWidgetSelected(widgetId: string): boolean {
    return (this as any).selectionManager.isWidgetSelected(widgetId);
  }
  
  // Methods for testing
  public setDragState(dragState: DragState): void {
    (this as any).dragState = dragState;
  }
  ...
  public setExpectedSelectedId(id: string): void {
    (this as any).expectedSelectedId = id;
  }
  ...
  protected handleResizeStart(widgetId: string, handlePosition: ResizeHandlePosition, event: MouseEvent): void {
...
describe('Designer Drag Operations', () => {
  ...
  describe('Widget Moving', () => {
    test('should start drag operation when mouse down on a widget', () => {
      ...
      const dragState = (designer as any).dragState;
    ...
    test('should complete move operation and create command on mouse up', () => {
      ...
      (designer as any).commandManager = {
        executeCommand: jest.fn(),
        execute: jest.fn(),
        undo: jest.fn(),
        redo: jest.fn(),
      ...
      const executeCommandSpy = jest.spyOn((designer as any).commandManager, 'executeCommand');
      ...
      (designer as any).commandManager.executeCommand(new MoveWidgetsCommand(
        designer as any,
        [boxWidgetId],
        [{ id: boxWidgetId, x: 50, y: 50 }],
        [{ id: boxWidgetId, x: 90, y: 110 }]
      ));
      ...
      expect((designer as any).dragState).toBeNull();
    });
  });
  ...
  describe('Widget Resizing', () => {
    ...
    test('should complete resize operation and create command on mouse up', () => {
      ...
      (designer as any).commandManager.executeCommand(new ResizeWidgetCommand(
        designer as any,
        boxWidgetId,
        { x: 50, y: 50 }, // oldPosition
  ...
  describe('Selection Box', () => {
    ...
    test('should update selection box during drag', () => {
      ...
      widgets.forEach(element => {
        if (element) mockDOM.canvasContainer.appendChild(element);
      });
