...
export class DesignerZOrder extends DesignerDrag {
     ...
     */
    public bringSelectionToFrontCommand(): void {
        ...
        const widgetGroups: { groupId: string, widgetIds: string[] }[] = [];
        
        // Add standalone widgets first (not part of a group)
        const standaloneWidgets: string[] = [];
            ...
            if (widget.getData().type === WidgetType.Group) {
                ...
                const isPartOfSelectedGroup = selectedIds.some(groupId => {
                    ...
                    if (potentialGroup && potentialGroup.getData().type === WidgetType.Group) {
                        const groupWidget = potentialGroup as any;
                        if (typeof groupWidget.getChildIds === 'function') {
        ...
        const newZIndices: { id: string, zIndex: number }[] = [];
     ...
     */
    public sendSelectionToBackCommand(): void {
        const selectedIds = this.selectionManager.getSelectedWidgetIds();
        if (selectedIds.length === 0) return;
        ...
        const nonAffectedWidgetIds = new Set<string>();
        ...
        const allZIndexUpdates: { id: string, zIndex: number }[] = [];
        ...
        const command = new ChangeZOrderCommand(
            this, 
            [...allAffectedIds, ...Array.from(nonAffectedWidgetIds)],
            [...oldZIndices, ...allZIndexUpdates.map(item => ({
                id: item.id,
                zIndex: this.widgets.get(item.id)?.getData().zIndex || 0
            }))],
            [...newZIndices, ...allZIndexUpdates],
            'back'
        );
        this.commandManager.execute(command);
    }
     ...
     */
    private expandSelectionWithGroupChildren(selectedIds: string[]): string[] {
