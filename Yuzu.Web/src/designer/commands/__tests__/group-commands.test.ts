...
class MockWidget {
  ...
  getRect(useDom: boolean = false): any {
    return { 
...
class MockDesigner {
  ...
  widgets: Map<string, MockWidget> = new Map();
  ...
  createGroupWidget(childIds: string[], groupId: string = 'group-1'): MockGroupWidget {
    const groupData = {
      id: groupId,
      position: { x: 10, y: 10 },
  ...
  addCreatedWidget(widget: any): void {
...
jest.mock('../group-commands.js', () => {
  ...
  return {
    ...originalModule,
    GroupWidgetsCommand: jest.fn().mockImplementation((widgetIds, designer, selectionCallback) => {
      let groupId = 'group-1';
      let executed = false;
...
describe('Group Commands', () => {
  let mockDesigner: MockDesigner;
  let selectionCallback: jest.Mock;
  ...
  describe('GroupWidgetsCommand', () => {
    test('should create a group widget when executed', () => {
      ...
      const command = new GroupWidgetsCommand(widgetIds, mockDesigner as any, selectionCallback);
  ...
  describe('UngroupWidgetsCommand', () => {
    test('should remove group and restore child widgets when executed', () => {
      ...
      const command = new UngroupWidgetsCommand([groupId], mockDesigner as any, selectionCallback);
    ...
    test('should handle multiple groups', () => {
      ...
      const command = new UngroupWidgetsCommand(['group-1', 'group-2'], mockDesigner as any, selectionCallback);
