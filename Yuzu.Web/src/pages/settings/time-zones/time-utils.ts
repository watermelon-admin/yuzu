 ...
 */
export function updateTimeZoneInfoTime(timeZone: TimeZoneInfo): void {
 ...
 */
export function updateElementTextIfDifferent(elementId: string, newText: string): void {
 ...
 */
export function formatUtcOffset(timeZone: TimeZoneInfo): string {
    return `UTC ${timeZone.utcOffsetHours >= 0 ? '+' : ''}${timeZone.utcOffsetHours}${
        timeZone.utcOffsetMinutes ? ':' + timeZone.utcOffsetMinutes.toString().padStart(2, '0') : ':00'
    }`;
}