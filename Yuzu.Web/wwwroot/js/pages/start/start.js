...
class BreakButtonInfo {
    constructor(intervalMultiplier, intervalMinutes, breakTypeId, breakDuration, isOtherButton) {
 ...
 */
export function updateHeadlineTime(currentTime) {
 ...
 */
export function parseBreakButtonData(sourceElement) {
    ...
    let breakButtonInfo = new BreakButtonInfo(parseInt(sourceElement.getAttribute('data-interval-multiplier')), parseInt(sourceElement.getAttribute('data-interval-minutes')), breakId, parseInt(sourceElement.getAttribute('data-break-duration')), sourceElement.classList.contains('other-button'));
 ...
 * @param {luxon.DateTime} currentTime - The current local time.
 */
export function updateEndTimeButtonTimes(currentTime) {
    ...
    elementsWithDataAttribute.forEach(element => {
        ...
        if (breakButtonInfo.isOtherButton)
            return;
        element.innerHTML = TimeTools.roundDateTime(currentTime, breakButtonInfo.intervalMinutes, breakButtonInfo.intervalMultiplier)
 ...
 */
export function handlePresetButtonClick(element) {
    // Parse the data from the clicked button
    const breakButtonInfo = parseBreakButtonData(element);
    console.log(breakButtonInfo);
    // Calculate the end time for the break
    const breakEndTimeUnixTimestamp = calculateBreakEndTimeUnixTimestamp(TimeTools.GetCurrentLocalTime(), breakButtonInfo.intervalMinutes, breakButtonInfo.intervalMultiplier, breakButtonInfo.breakDuration);
 ...
 * @param {HTMLElement} element - The clicked button element.
 */
export function handleOtherButtonClick(element) {
 ...
 */
export function calculateBreakEndTimeUnixTimestamp(currentLocalTime, intervalMinutes, intervalMultiplier, breakDuration) {
 ...
 */
export function setBreakEndTimeField(breakEndTimeUnixTimestamp) {
    const breakEndTimeField = document.querySelector("input[name='Input.BreakEndTimeUnixTimestamp']");
 ...
 */
export function setBreakTypeIdField(breakTypeId) {
    const breakTypeIdField = document.querySelector("input[name='Input.BreakTypeId']");
    if (breakTypeIdField) {
 ...
 * @param {string} breakTypeId - The ID of the break type.
 */
export function setFormValues(breakEndTimeUnixTimestamp, breakTypeId) {
 ...
 * @param {luxon.DateTime} currentTime - The current local time.
 */
export function updateTimeDisplayFields(currentTime) {
...
window.updateHeadlineTime = updateHeadlineTime;
window.updateEndTimeButtonTimes = updateEndTimeButtonTimes;
window.parseBreakButtonData = parseBreakButtonData;
window.handlePresetButtonClick = handlePresetButtonClick;
window.handleOtherButtonClick = handleOtherButtonClick;
window.calculateBreakEndTimeUnixTimestamp = calculateBreakEndTimeUnixTimestamp;
window.setFormValues = setFormValues;
window.InitPage = InitPage;
window.MinuteChanged = handleMinuteChange;
window.checkMinuteChange = checkMinuteChange;
window.UpdateTimeDisplayFields = updateTimeDisplayFields;
