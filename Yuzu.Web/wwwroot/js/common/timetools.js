...
export function GetCurrentLocalTime() {
    return luxon.DateTime.now().setZone(document.getElementById("homeTimeZoneCode").value);
}
/**
 * Retrieves a local time based on the time zone code.
 * @returns {luxon.DateTime} The current local time in that TZID.
 */
export function GetLocalTimeByTZID(TZID, dateTime) {
 ...
 * @returns {luxon.DateTime} The current local time in that TZID.
 */
export function GetLocalTimeByTimestamp(TZID, unixTimestamp) {
 ...
 */
function areHourAndMinuteEqual(dateTime1, dateTime2) {
 ...
 */
function nearestMultipleBefore(baseTime, intervalMinutes) {
 ...
 */
export function nearestMultipleAfter(baseTime, intervalMinutes) {
    // Get the number of minutes elapsed since the start of the hour
    const minutesElapsed = baseTime.minute + (baseTime.hour * 60);
...
* roundDateTime(new Date(), 15, 1);
*/
export function roundDateTime(baseDateTime, intervalMinutes, intervalMultiplier) {
    let roundedDateTime;
...
export function replacePlaceholders(templatedMessage, values) {
...
export function createExampleReplacement(template) {
