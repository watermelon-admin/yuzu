# Script to generate timezone data from IANA timezone database
Write-Host "Starting timezone data generation..."

# Create output file
$outputPath = Join-Path $PSScriptRoot "timezones.txt"
Write-Host "Creating output file: $outputPath"

# Write header
@"
# Timezone data generated from IANA timezone database (https://www.iana.org/time-zones)
# Country names simplified from UN Terminology Database (https://unterm.un.org)
# City names from UN/LOCODE (https://unece.org/trade/cefact/unlocode-code-list-country-and-territory)
# Continental regions from UN Statistics Division (https://unstats.un.org/unsd/methodology/m49/)
# Generated on: $(Get-Date -Format "yyyy-MM-dd HH:mm:ss")
# Format: IanaId|Continent|City|Country
"@ | Set-Content $outputPath

# Define timezone data - strictly following IANA timezone database
$timezones = @(
    # Africa (UN region code 002)
    "Africa/Abidjan|Africa|Abidjan|Ivory Coast",
    ...
    # Americas (UN region codes 019, 419)
    "America/Anchorage|America|Anchorage|United States",
    ...
    "America/Chicago|America|Chicago|United States",
    "America/Denver|America|Denver|United States",
    "America/Godthab|America|Nuuk|Greenland",
    "America/Halifax|America|Halifax|Canada",
    "America/Lima|America|Lima|Peru",
    "America/Los_Angeles|America|Los Angeles|United States",
    "America/Mexico_City|America|Mexico City|Mexico",
    "America/New_York|America|New York|United States",
    "America/Phoenix|America|Phoenix|United States",
    "America/Santiago|America|Santiago|Chile",
    "America/Sao_Paulo|America|Sao Paulo|Brazil",
    "America/St_Johns|America|St. John's|Canada",
    "America/Toronto|America|Toronto|Canada",

    # Asia (UN region code 142)
    ...
    "Asia/Brunei|Asia|Bandar Seri Begawan|Brunei",
    ...
    # Oceania (UN region code 009)
    "Pacific/Auckland|Pacific|Auckland|New Zealand",
    "Pacific/Fiji|Pacific|Suva|Fiji",
    "Pacific/Guam|Pacific|Hagatna|Guam",
    "Pacific/Honolulu|Pacific|Honolulu|United States",
    "Pacific/Port_Moresby|Pacific|Port Moresby|Papua New Guinea",
    "Pacific/Tongatapu|Pacific|Nuku'alofa|Tonga",

    # Australia (treated separately in UN M.49)
...
Write-Host "Done! Generated $($timezones.Count) timezone entries." 